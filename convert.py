...
class DarkNetParser(object):
    ...
    def conv(self, block):
        '''When reading darknet's yolov3.weights file, the order is 1 - bias； 2 - If there is a bn, read scale，mean，var 3 - Get the weights '''
    ...
    def bn(self,filters,conv_bias):
        '''bn has 4 parameters. they're bias, scale, mean, var, The bias has been read, and the remaining three are read here, scale, mean, var '''
           
        bn_weights = self.weight_loader.parser_buffer(
                              shape=(3,filters),
                              dtype='float32',
                              buffer_size=(filters*3)*4)
        # scale, bias, mean,var         bn_weight_list = [bn_weights[0],
                          conv_bias,
                          bn_weights[1],
                          bn_weights[2] ]
